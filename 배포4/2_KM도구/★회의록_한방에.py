# -*- coding: utf-8 -*-
r"""★회의록 한 방에 — 이 파일만 더블클릭하십시오. (2026-09-22)
...
def load_t52(tools):
    ...
    try:
        ...
        if not hasattr(MB, 'is_site'):
            raise ImportError('52번이 옛 판(v1)입니다')      # v2 부터 현장명 절대 규칙이 있다
        return MB, ''
    except Exception:
        pass
...
def main():
    ...
    def s4():
        ...
        say('  올렸습니다 : %s' % tgt)
        # 즉시발송(앱스 스크립트)이 깔려 있는지 보고 말한다. 없는데 「간다」고 하면 거짓말이 된다.
        if os.path.isfile(os.path.join(dest, '즉시발송_로그.csv')):
            say('  5분 안에 bsy@micronic.co.kr 로 갑니다. (KM_즉시발송 이 지켜보고 있습니다)')
            sent[0] = True
        else:
            say('')
            say('  ※ 메일은 아직 저절로 가지 않습니다.')
            say('    앱스 스크립트(KM_즉시발송)를 아직 안 까셨습니다. 한 번만 까시면 끝입니다.')
            say('    지금 받으시려면 클로드 대화창에 「올렸어」 라고만 하십시오.')
        return tgt
    ...
    if r3 and r3.get('회의수'):
        ...
        if sent[0]:
            ...
            say(' 메일을 받으시려면 클로드 대화창에 「올렸어」 라고만 하십시오.')
            say(' (앱스 스크립트를 한 번 까시면 그다음부터는 저절로 갑니다)')
