...
def build(quiet=True):
    ...
    # v46 : 「진행중」 - 손 대셨지만 아직 안 끝난 것. 완료로 닫지 않고 계속 띄운다.
    md += ['', '## 진행중 (손 대셨고 아직 안 끝난 것 — 끝나면 「완료」 한마디)']
    _wip = [d for d in plan_rows if (d.get('상태') or '').strip() == '진행중']
    md += ['- %s %s · %s%s' % ((d.get('코드') or ''), d['현장'], d['할일'], (' → ' + d['누가']) if d['누가'] else '') for d in _wip] or ['- (없음)']
    # v46 : 「만들어줘」 를 받으면 바로 만들지 않고, 어떻게 만들지 먼저 여쭙는다.
    md += ['', '## 제가 이렇게 만들겠습니다 (맞으면 「맞아」, 아니면 한 줄 고쳐 주십시오)']
    _mw = [d for d in plan_rows if (d.get('상태') or '').strip() == '만들기대기']
    md += ['- %s %s · %s — 「%s」 로 만들겠습니다' % ((d.get('코드') or ''), d['현장'], d['할일'], (d.get('만들기') or d['할일'])) for d in _mw] or ['- (없음)']
