...
# 요율 표준 출발값 (성윤 차장 확정 2026-08-13, 2026-09-02 재확인).
# 시트의 「■ 요율 입력」 노란 칸에 기본값으로만 채우고, 계산은 전부 그 칸을 참조한다.
# 현장 규모·발주처에 따라 차장님이 칸을 고치면 전체가 따라간다. 구 160%(계약×1.6)는 폐기.
RATE_ASSEMBLY = 0.3   # 조립비율 (CB 내부 자재비 × 이 비율, 외함 제외)
RATE_CONTRACT = 1.5   # 계약 실행가 = 실행 × 1.5
RATE_QUOTE = 1.8      # 견적가 = 실행 × 1.8
RATE_BUDGET = 2.1     # 설계 예산가 = 실행 × 2.1
