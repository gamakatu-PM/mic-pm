...
function renderMap(){
  ...
  el.innerHTML=renderFeedBar()+`<details class="notice" ${localStorage.getItem('km_help_map')==='off'?'':'open'} ontoggle="try{localStorage.setItem('km_help_map',this.open?'on':'off')}catch(e){}"><summary style="cursor:pointer;font-weight:600">이 화면 설명</summary>현장마다 9단계 여정입니다. 단계를 누르면 <b>무엇이 필요하고 · 지금 어디까지 왔고 · 제가 무엇을 만들어 드릴 수 있는지</b>가 나옵니다. 열쇠(필요한 것)를 채우면 단계가 닫힙니다.</details>
